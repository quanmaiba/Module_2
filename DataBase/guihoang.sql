...
--Cập nhật thêm dữ liệu cho bảng GIAOVIEN
	...
	--lấy ra mã GV, tên GV và tên khoa của GV đó
	SELECT * FROM dbo.GIAOVIEN,dbo.KHOA;
	SELECT gv.MAGV, gv.HOTEN, k.TENKHOA 
	FROM dbo.GIAOVIEN gv 
	inner JOIN  dbo.BOMON bm On gv.MABM=bm.MABM  
	FULL JOIN dbo.KHOA k ON k.MAKHOA = bm.MAKHOA AND k.TENKHOA = N'công nghệ thông tin';

	SELECT distinct HOTEN, MAGV, BOMON.MABM FROM dbo.GIAOVIEN,dbo.BOMON WHERE MAGV = TRUONGBM; 
	SELECT * FROM dbo.GIAOVIEN gv INNER JOIN dbo.BOMON bm ON bm.MABM = gv.MABM;
	
	SELECT GV2.HOTEN, GV1.HOTEN 
	FROM dbo.GIAOVIEN GV1
	 JOIN dbo.GIAOVIEN GV2
	ON GV1.GVQLCM = GV2.MAGV

	SELECT GV1.HOTEN AS N'Người quản lý',GV2.HOTEN
	...
	(
     ...
     SELECT * FROM dbo.KHOA K , dbo.BOMON bm
	 ...
	 WHERE 2< 
	 (
		SELECT COUNT(*) FROM dbo.BOMON BM 
		JOIN dbo.GIAOVIEN GV ON GV.MABM = BM.MABM 
		AND BM.MAKHOA = K.MAKHOA
	 )

	 SELECT * FROM dbo.KHOA K ,dbo.BOMON BM , dbo.GIAOVIEN GV 
	 WHERE BM.MAKHOA = K.MAKHOA 
	 AND BM.MABM = GV.MABM

	 SELECT * FROM dbo.GIAOVIEN GVT 
	 ORDER BY GVT.NGSINH DESC 
	 WHERE 
	 (
		SELECT TOP(5)*  FROM dbo.GIAOVIEN GTV2
		WHERE 
	 )
	 ...
	 SELECT nt.MAGV,gv.HOTEN FROM dbo.GIAOVIEN gv 
	 JOIN dbo.NGUOITHAN nt
	 ON nt.MAGV = gv.MAGV
	 GROUP BY nt.MAGV,gv.HOTEN
	 HAVING count(nt.MAGV) > 1

	 SELECT BM.TENBM, COUNT(*) FROM dbo.BOMON BM ,dbo.GIAOVIEN GV
	 GROUP BY BM.TENBM

	 SELECT BM.TENBM,BM.MAKHOA, COUNT(*) FROM dbo.BOMON BM ,dbo.GIAOVIEN GV
	 WHERE BM.MABM=GV.MABM
	 GROUP BY BM.TENBM,BM.MAKHOA
	 
	 SELECT * FROM dbo.GIAOVIEN 
	 WHERE lUONG > 
	 (SELECT SUM(GV.LUONG) FROM dbo.GIAOVIEN GV)/ (SELECT COUNT(*) FROM dbo.GIAOVIEN)

	 SELECT AVG(LUONG) FROM dbo.GIAOVIEN 	 
	 GROUP BY LUONG 
	  HAVING LUONG > AVG(LUONG); 

	  SELECT GV.HOTEN , COUNT(*) FROM dbo.GIAOVIEN GV , dbo.THAMGIADT DT 
	  WHERE GV.MAGV = DT.MAGV
	  GROUP BY DT.MAGV , GV.HOTEN;

	  SELECT GV.HOTEN,COUNT(*) FROM dbo.GIAOVIEN GV , dbo.NGUOITHAN NT
	  WHERE GV.MAGV = NT.MAGV
	  ...
	  SELECT GV.HOTEN, COUNT(*) FROM dbo.GIAOVIEN GV , dbo.THAMGIADT DT
	  WHERE GV.MAGV = DT.MAGV 
	  GROUP BY  DT.MADT,GV.HOTEN

	  SELECT * FROM dbo.GIAOVIEN;
	  SELECT * FROM dbo.BOMON;
	  SELECT * FROM dbo.KHOA;

	  SELECT K.TENKHOA , COUNT(*) FROM dbo.KHOA K , dbo.BOMON BM
	  WHERE K.MAKHOA = BM.MAKHOA 
	  GROUP BY K.MAKHOA, K.TENKHOA
	  HAVING COUNT(BM.MAKHOA) > COUNT(*);
	  ...
	  SELECT   ISNULL(KINHPHI,2) FROM dbo.DETAI
	  SELECT * FROM dbo.BOMON;
	  SELECT * FROM dbo.GIAOVIEN;
...
--Xuất ra thông tin giáo viên và Giáo viên quản lý chủ nhiệm của người đó
	SELECT gv1.HOTEN , gv2.HOTEN AS nguoiquanLy
	FROM dbo.GIAOVIEN gv1 
	LEFT JOIN dbo.GIAOVIEN gv2 
	ON gv1.GVQLCM = gv2.MAGV
	SELECT * FROM dbo.GIAOVIEN;
...
FROM dbo.KHOA k 
JOIN dbo.BOMON bm 
ON bm.MAKHOA = k.MAKHOA AND bm.MAKHOA = 'CNTT'
...
DECLARE @Ketqua CHAR(10) = N'Đạt'
SELECT DISTINCT gv.MAGV, gv.HOTEN, dt.TENDT , tg.KETQUA
FROM  dbo.GIAOVIEN gv
JOIN dbo.THAMGIADT tg
ON tg.MAGV = gv.MAGV 
AND tg.KETQUA = @Ketqua
JOIN dbo.DETAI dt
ON dt.MADT = tg.MADT; 

GO 
CREATE PROC updatetest
@Magv CHAR(10) , @luong int 
AS 
BEGIN 
SELECT MAGV,LUONG FROM dbo.GIAOVIEN WHERE LUONG = @luong;
END
GO  	  
EXEC updatetest @Magv = N'003' , @luong = 1600
GO 
 DROP PROC dbo.updatetest1;
 GO	
...
GO 
CREATE VIEW VIEW_ten 
AS 


SELECT * 
FROM dbo.BOMON,dbo.DETAI;
GO 

SELECT * FROM  VIEW_ten
GO	

CREATE PROC updatetest1
@Magv CHAR(10) , @luong int 
AS 
BEGIN 
...
END
GO  	  
EXEC updatetest1 @Magv = N'003' , @luong = 2200
GO 
